...
public class SubtransExecutorTest {
   ...
   */
  @Test
  public void blockOnAquiringLastPermit() {
    final ExecutorService executorService = Executors.newFixedThreadPool( 1 );

    SubtransExecutor subtransExecutor = new SubtransExecutor( "subtransname", null, null, false, null, "", 1 );

    //Prefetch count is set to 1, so blocking occurs when that single permit is acquired
    Future future = executorService.submit( () -> safeAcquirePermits( subtransExecutor, 1 ) );
  ...
  @Test
  public void blockAndUnblockTwice() throws KettleException, InterruptedException {
    final ExecutorService executorService = Executors.newFixedThreadPool( 1 );
    ...
    Future future = executorService.submit( () -> safeAcquirePermits( subtransExecutor, 8 ) );

    //Acquire the permits before releasing them by waiting briefly for the thread to spin up
    Thread.sleep( 1000 );
    ...
    final boolean timedOut = !safeWaitForCompletion( future, 3 );
